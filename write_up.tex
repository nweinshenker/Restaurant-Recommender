...
\end{enumerate}
Now lets assume that for  k-\textbf{nearest neighbor classification } we are using  euclidean distance; X:training data; Y: class labels of X; x:unknown sample.
Our k-nearest classifier will be as follow:
Let $d$ be a distance measure for our dataset $D$ where
\begin{equation}
...
\begin{equation}
\text{ (i.e., the majority class with repect to the classes of the neighbors)}
\end{equation}
